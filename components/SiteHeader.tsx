  ...
  return (
    <header className="border-y border-line bg-graphite">
      <div className="mx-auto w-full max-w-4xl px-2 sm:px-4">
        <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 py-1 text-sm">
          <Link href="/" className="font-mono text-data text-fg hover:text-accent-text">
            PETERLAW.DEV
          </Link>
        ...
        <nav
          aria-label="Sections"
          className="flex flex-wrap gap-x-4 gap-y-1 border-t border-line py-1 text-sm"
        >
          {sections.map((section) => (
            ...
            >
              <span className="font-mono text-data text-fg group-hover:text-accent-text">
                {section.index}
              </span>{" "}
              {section.label}
            </a>
